...
async function resolveAndPlay(player, query) {
  ...
  if (result.loadType === "search" || result.loadType === "track") {
    ...
    throw new Error(
      result.data?.message || "Lavalink could not load that query."
    );
  }

  if (!tracks.length) {
    throw new Error("Nothing playable was found.");
  }
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  try {
    if (interaction.commandName === "play") {
      ...
      const result = await resolveAndPlay(
        player,
    ...
    if (interaction.commandName === "nowplaying") {
      ...
      if (s.current.artworkUrl) {
        e.setThumbnail(s.current.artworkUrl);
      }
    ...
    if (interaction.commandName === "loop") {
      player.loop = interaction.options.getString(
        "mode",
        true
      );
