...
app.get("/api/me", requireAuth, (req, res) => {
  res.json({
    ...
    guilds: req.session.user.guilds.filter(g => bot.guilds.cache.has(g.id))
  });
});
...
app.get("/api/player/:guildId", requireAuth, async (req, res) => {
  try {
    const { music } = await import("../src/index.js");
    res.json(music.get(req.params.guildId).state());
  } catch {
    res.json({ guildId: req.params.guildId, connected: false, current: null, queue: [] });
  }
});
...
app.post("/api/player/:guildId/action", requireAuth, async (req, res) => {
  ...
  try {
    ...
    io.to(req.params.guildId).emit("player:update", player.state());
...
io.on("connection", socket => {
  socket.on("guild:watch", guildId => socket.join(guildId));
});
